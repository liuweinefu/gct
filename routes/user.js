    ...
    switch (req.body.postType) {
        case 'add':
            filtrateImportPostData(req.body.postData)
                ...
                .catch(function(err) {
                    console.log('user/inmportExcel/add Err------------------------------------------start');
                    console.log(err);
                    console.log('user/inmportExcel/add Err------------------------------------------end');
                    res.json({
                            err: true,
                            message: '添加失败',
        ...
        case 'update':
            filtrateImportPostData(req.body.postData)
                ...
                .catch(function(err) {
                    console.log('user/inmportExcel/update Err------------------------------------------start');
                    console.log(err);
                    console.log('user/inmportExcel/update Err------------------------------------------end');
                    res.json({
                            err: true,
                            message: '更新失败',
        ...
        case 'addAndUpdate':
            filtrateImportPostData(req.body.postData)
                ...
                .catch(function(err) {
                    console.log('user/inmportExcel/addAndUpdate Err------------------------------------------start');
                    console.log(err);
                    console.log('user/inmportExcel/addAndUpdate Err------------------------------------------end');
                    res.json({
                            err: true,
                            message: '添加或更新失败',
        ...
        case 'delete':
            filtrateImportPostData(req.body.postData)
                ...
                .catch(function(err) {
                    console.log('user/inmportExcel/delete Err------------------------------------------start');
                    console.log(err);
                    console.log('user/inmportExcel/delete Err------------------------------------------end');
                    res.json({
                            err: true,
                            message: '更新删除',
        ...
        case 'copy':
            filtrateImportPostData(req.body.postData)
                .then(function(Data) {
                    ...
                    return executeQueries(['delete from user'].concat(insertQueries(copyData)), true);
                })
                .then(function(value) {
                    let backNumber = 0;
                    if (F.isNotEmpty(value) && Array.isArray(value)) {
...
router.post('/', function(req, res, next) {
    ...
    let selectQueries = [];
    selectQueries.push('select count(*) as count from view_user_role');
    ...
    if (F.isEmpty(req.body.name) || F.isEmpty(req.body.value)) {
        query = 'SELECT id,name,CONCAT(role_id," ",role_name) as role,nfxyzy,workunit FROM view_user_role limit ' + mysqlPool.escape(offset) + ',' + mysqlPool.escape(rows);
    } else {
        query = 'SELECT id,name,CONCAT(role_id," ",role_name) as role,nfxyzy,workunit FROM view_user_role where ' + mysqlPool.escapeId(req.body.name) + ' like "%' + req.body.value.trim() + '%" limit ' + mysqlPool.escape(offset) + ',' + mysqlPool.escape(rows);
    };
    ...
    executeQueries(selectQueries, true)
        ...
        .catch(function(err) {
            console.log('user/ Err------------------------------------------start');
            console.log(err);
            console.log('user/ Err------------------------------------------end');
            res.status(500).end();
        });
});
...
router.post('/save', function(req, res, next) {
    ...
    Promise.all(executePromise)
        ...
        .catch(function(err) {
            console.log('user/save Err------------------------------------------start');
            console.log(err);
            console.log('user/save Err------------------------------------------end');
            res.status(500).end();
        });
...
router.post('/roles', function(req, res, next) {
    let selectRolesQueries = [];
    if (F.isEmpty(req.body.q)) {
        selectRolesQueries.push('SELECT CONCAT(id," ",name) as role FROM role order by id');
    } else {
        let searchQuery = '%' + mysqlPool.escape(req.body.q).trim() + '%';
        selectRolesQueries.push('SELECT ,CONCAT(id," ",name) as role FROM role where name like ' + searchQuery + ' or id like ' + searchQuery + ' order by id');
    }

    executeQueries(selectRolesQueries, true)
        .then(function(rows) {
            res.json(rows);
        })
        .catch(function(err) {
            console.log('user/roles Err------------------------------------------start');
            console.log(err);
            console.log('user/roles Err------------------------------------------end');
            res.status(500).end();
        });
...
router.get('/exportExcel', function(req, res, next) {
    ...
    mysqlPool.getConnectionAsync()
        .then(function(con) {
            ...
            return currentCon.queryAsync('select * from view_user_role');
        })
        .then(function(result) {
            currentCon.release();
            let data = [];
            //let keys = Object.keys(result[0]);
            let keys = ['id', 'name', 'role_id', 'role_name', 'nfxyzy', 'workunit'];
            data.push(keys);
        ...
        .then(function(data) {
            let wirteBuffer = xlsx.build([{ name: "jobwork_user", data: data }])
...
var insertQueries = function(arrayData) {
    ...
    for (item of arrayData) {
        if (F.isEmpty(item.name)) { continue; }
        item.role_id = F.isEmpty(item.role) ? '0' : (item.role.trim())[0];
        let query = ''
        for (let key in item) {
            switch (key) {
                case 'id':
                ...
                case 'role_name':
                    break;
                case 'pass': //默认用户密码，用户名+'8888'
                    item.pass = F.isEmpty(item.pass) ? crypto.createHash('md5').update(item.name.trim() + '8888').digest('hex') : crypto.createHash('md5').update(item.pass).digest('hex');
                default:
...
var updateQueries = function(arrayData, keys) {
    ...
    for (item of arrayData) {
        ...
        for (let key in item) {
            switch (key) {
                ...
                case 'name':
                    ...
                    break;
                case 'pass':
                    if (F.isNotEmpty(item.pass)) {
                        item.pass = crypto.createHash('md5').update(item.pass.toString().trim()).digest('hex');
                        query = query + mysqlPool.escapeId(key) + '=' + mysqlPool.escape(item[key]) + ',';
                    }
                    break;
                default:
        ...
        // queries.push('update user set ' + query.slice(0, -1) + ' where id=' + mysqlPool.escape([user.id]));
